...
module data
  ! parameter that defines double precision reals
  ...
  complex(kind=dp),parameter :: iunit=cmplx(0.0_dp,1.0_dp,kind=dp)
  ...
  real(kind=dp),allocatable :: isotope_mass(:)
  real(kind=dp),allocatable :: isotope_abundance(:)
...
contains
  ...
  subroutine data_calc_mandg(element,m,g)
    ...
    do i=1,niso
       if(isotope_element(i).eq.element) then
          g=g+isotope_abundance(i)*(1.0_dp-isotope_mass(i)/m)**2
       end if
    end do
    g=g/100.0_dp
